...
function getColorFromUnit(unit: any): number {
  const defaultColor = 0x80000000;
  if (unit.colorR === null || unit.colorG === null || unit.colorB === null) {
    return defaultColor;
  }
  const secondByteOffset = 16;
  const firstByteOffset = 8;
  const res: number = (unit.colorR << secondByteOffset) | (unit.colorG << firstByteOffset) | unit.colorB;
  return res;
}
const unnasignedMaxDomain = 100;
const unnassignedQuantatize = 10;
const unassignedColorScale = scaleOrdinal<number, string>()
  .domain([0, unnasignedMaxDomain])
  .range(quantize(interpolateRgb('#e6f1cf', '#85906d'), unnassignedQuantatize));
...
function getBaseValue(index: number, surfaces: any[], datapoint: number): any {
  ...
  const areas: any = surfaces.reduce((acc: any, surface: any, i: number) => {
    if (!surface.isBase) {
      ...
      acc[surface.group].push({
        id: surface.name,
        label: surface.name,
        color: (surface.unit && getColorFromUnit(surface.unit)) || defaultColor,
