...
function simplify(inputArr: number[][], maxOffset = 0.001, maxDistance = 10): number[][] {
  const minimumInputLengthToSimplify = 4;
  if (inputArr.length <= minimumInputLengthToSimplify) {
    return inputArr;
  }
...
export function generateProjectedTrajectory(poslog: SurveySample[], defaultIntersectionAngle: number): number[][] {
  ...
  const interpolator: any = new CurveInterpolator(points, tension, numPoints);
  ...
  if (relativeDist < thresholdRelativeDist) {
    const radCurtainDirection = (defaultIntersectionAngle / DEGRESS_IN_CIRCLE / 2) * Math.PI;
  ...
  // const angle: number = ((a1 > 0 ? a1 : 2 * Math.PI + a1) * DEGRESS_IN_CIRCLE) / (2 * Math.PI);
